...
#include <arpa/inet.h>
#include <netdb.h>
#define MYPORT "21159"    // the port users will be connecting to
...
#define SERVER_2_PORT "22159" // the port client will be connecting to 
...
//  printf("The Server 1 has UDP port number %d and IP address %s\n",(unsigned)ntohs(udpport.sin_port),inet_ntoa(*((struct in_addr *)"nunki.usc.edu")));
  ...
  //Onscreen message
	...
	if(flag==1)
	{
	printf("The Server 1 sends the reply %s to Client 1 with port number _____ and IP address ____.",temp);
  ...
  //  printf("client: connecting to %s\n", s);
    ...
    printf("The Server 1 has received key %s \n",buf1);
