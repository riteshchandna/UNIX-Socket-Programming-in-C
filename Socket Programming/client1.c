...
bind(sockfd,(struct sockaddr *)&adr_inet,sizeof(adr_inet));
    ...
    printf("The Client1 sends the request %s to the Server1 with port number %d and IP address %s.\n",msg,PORT,inet_ntoa(ipadrs));
  ...
  printf("The Client1 received the value %s from the Server1 with port number %d and IP address %s.\n",val2,PORT,inet_ntoa(ipadrs));
  printf("The client1's port number is %d and the IP address is %s\n ",(unsigned)ntohs(udpt.sin_port),inet_ntoa(ipadrs));
